...
def process_all_apps(project_root, output_dir):
    ...
    for app_name in os.listdir(project_root):
        ...
        if os.path.isdir(app_path) and os.path.exists(os.path.join(app_path, '__init__.py')):
            # Definir el archivo de salida dentro de la carpeta de salida
            output_file = os.path.join(output_dir, f"{app_name}_combined.py")
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(project_root, 'salida')  # Carpeta donde se guardarán los archivos combinados
    process_all_apps(project_root, output_dir)
